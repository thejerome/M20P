    ...
    public CheckingSingleConditionResult checkSingleCondition(ConditionForChecking condition, String instructions, GeneratingResult generatingResult) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < checkTask.getQ().length; i++){
                if (! (checkTask.getQ()[i].compareTo(varInstr.getQ()[i]) == 0)){
                    checkQ = false;
                }
                if (! (varInstr.getQHatch()[i].subtract(checkTask.getQHatch()[i]).abs().compareTo(varInstr.getQHatch()[i].multiply(new BigDecimal("0.02"))) <= 0)){
                    checkQHatch = false;
                }
                if (! (varInstr.getQ2Hatch()[i].subtract(checkTask.getQ2Hatch()[i]).abs().compareTo(varInstr.getQ2Hatch()[i].multiply(new BigDecimal("0.02"))) <= 0)){
                    checkQ2Hatch = false;
                }
            }
